...
export const EuiNavMenu = ({ data, icon }) => {
    ...
    React.useEffect(() => {
        for (let i = 0; i < data.list.length; i++) {
            if (data.list[i].path === pathnames[1]) {
                handleOpen();
                break;
            } else {
                handleClose();
    ...
    return (
        <Mui.Stack>
            <EuiNavMenu.Parent onClick={handleClick} open={open} name={data.name || 'name'} icon={icon} />
            <Mui.Collapse in={open} timeout='auto' unmountOnExit>
                <Mui.Stack spacing={1}>
                    {data.list.map((nav, i) => {
                        return (
                            <Link key={i} to={nav.path || '/'}>
                                <EuiNavMenu.Chil linkName={nav.name} link={nav.path} />
                            </Link>
                        );
                    })}
                </Mui.Stack>
...
EuiNavMenu.Parent = function ({ name, open, icon, ...rest }) {
    return (
        <Style.SuiStack
            ...
            {...rest}
        >
            <Mui.Stack direction={'row'} justifyContent={'space-between'} alignItems={'center'} spacing={2}>
